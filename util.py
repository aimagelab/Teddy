...
class RandCheckpointScheduler(CheckpointScheduler):
    ...
    def step(self):
        return super(RandCheckpointScheduler, self).step(random.random())
...
class SineCheckpointScheduler(CheckpointScheduler):
    ...
    def step(self):
        alpha = math.sin((self.counter % self.period) / self.period * math.pi)
        self.counter += 1
        return super(SineCheckpointScheduler, self).step(alpha)
...
class SquareThresholdMSELoss(torch.nn.MSELoss):
    def __init__(self, threshold, reduction='mean'):
        super(SquareThresholdMSELoss, self).__init__(reduction=reduction)
...
class TextSampler:
    def __init__(self, corpus, words_per_line=32):
        ...
        self.unigram_counts = {k: 1 - (v / len(unigram_long_text)) for k, v in self.unigram_counts.items()}
        ...
        self.bigram_counts = {k: 1 - (v / len(bigram_long_text)) for k, v in self.bigram_counts.items()}
